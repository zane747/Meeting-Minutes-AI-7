...
GEMINI_AUDIO_PROMPT = """你是一位專業的會議記錄助理。請分析以下音檔，並以 JSON 格式回傳：
...
GEMINI_AUDIO_WITH_SPEAKERS_PROMPT = """你是一位專業的會議記錄助理。請分析以下音檔，並以 JSON 格式回傳。
...
GEMINI_SUMMARY_PROMPT = """你是一位專業的會議記錄助理。以下是一段會議的逐字稿，請分析後以 JSON 格式回傳：
