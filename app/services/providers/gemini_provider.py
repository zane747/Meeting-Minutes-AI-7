...
GEMINI_AUDIO_PROMPT = """你是一位專業的會議記錄助理。無論會議使用何種語言，你的所有輸出內容必須使用繁體中文（Traditional Chinese）。

請分析以下音檔，並以 JSON 格式回傳：
...
5. semantic_analysis：語意分析，包含：
   - topics：主題分段列表，每個主題包含 title（主題名稱）、start_time（起始時間 MM:SS）、end_time（結束時間 MM:SS）。根據內容自行決定分段數量與粒度。
   - utterance_intents：每段逐字稿的發言意圖列表，每項包含 utterance_index（從 0 開始的段落索引）和 intent（意圖標籤，例如「資訊分享」「提議」「提問」「回應」「決議」「反對」等，根據內容自行判斷合適的標籤）
...
請嚴格以下列 JSON 格式回傳：
{
  ...
  "semantic_analysis": {
    ...
    "utterance_intents": [
      {"utterance_index": 0, "intent": "..."}
    ]
  }
}"""

GEMINI_AUDIO_WITH_SPEAKERS_PROMPT = """你是一位專業的會議記錄助理。無論會議使用何種語言，你的所有輸出內容必須使用繁體中文（Traditional Chinese）。
...
5. semantic_analysis：語意分析，包含：
   - topics：主題分段列表，每個主題包含 title、start_time（MM:SS）、end_time（MM:SS）、speakers_involved（參與的說話者標籤列表）。根據內容自行決定分段數量。
   - speaker_summaries：每位說話者的訴求歸納，key 為說話者標籤（如 Speaker_0），value 包含 key_points（核心主張摘要）、stance（立場描述）、action_suggestions（提出的建議行動列表）
   - utterance_intents：每段發言的意圖，包含 utterance_index 和 intent（自行判斷合適的標籤）
...
請嚴格以 JSON 格式回傳：
{{"suggested_title": "...", "transcript": "...", "summary": "...", "action_items": [...], "semantic_analysis": {{"topics": [...], "speaker_summaries": {{}}, "utterance_intents": [...]}}}}"""

GEMINI_SUMMARY_PROMPT = """你是一位專業的會議記錄助理。無論會議使用何種語言，你的所有輸出內容必須使用繁體中文（Traditional Chinese）。

以下是一段會議的逐字稿，請分析後以 JSON 格式回傳：
...
4. semantic_analysis：語意分析，包含：
   - topics：主題分段列表，每個主題包含 title、start_time（MM:SS）、end_time（MM:SS）。根據內容自行決定分段數量。
   - utterance_intents：每段逐字稿的發言意圖，包含 utterance_index 和 intent（自行判斷合適的標籤）
...
請嚴格以 JSON 格式回傳：
{{"suggested_title": "...", "summary": "...", "action_items": [...], "semantic_analysis": {{"topics": [...], "utterance_intents": [...]}}}}"""
...
class GeminiProvider(AudioProcessor):
    ...
    async def _process_audio(self, file_path: str) -> ProcessingResult:
        ...
        response = model.generate_content(
            [GEMINI_AUDIO_PROMPT, audio_file],
    ...
    async def _summarize_transcript(self, transcript: str) -> ProcessingResult:
        ...
        """
        prompt = GEMINI_SUMMARY_PROMPT.format(transcript=transcript)
